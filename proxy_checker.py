...
async def checker(
    proxy_iterator: Iterable[Proxy],
    check_url: str,
    result_proxy_file: TextIO,
    timeout: int,
):
                ...
                async with session.get(check_url, **request_kwarg) as response:
                    if response.status == HTTPStatus.OK:
                        if (await response.text()).strip() == "PONG":
                            ...
                            logging.info(
                                "%s is bad: returns wrong data", proxy.get_formatted()
                            )
                    else:
                        logging.info(
                            "%s is bad: returns wrong http status %s",
                            proxy.get_formatted(),
                            response.status,
                        )
        except Exception as error:
            logging.info(
                "%s is bad: %s (%s)", proxy.get_formatted(), type(error), error
            )
...
@click.option(
    "--check-url",
    help='url to resource that used to check proxy, should contains text "PONG"',
    required=True,
)
@click.option(
    "--result-proxy-file",
    help="path to file with proxies have to be stored",
    required=True,
)
...
@click.option(
    "--protocol",
    help="override proxy format",
    type=click.Choice(["socks4", "socks5", "http", "https"], case_sensitive=False),
)
...
def main(
    ...
    proxies = load_proxies(
        proxy_file, proxy_url, protocol=protocol, custom_format=proxy_custom_format
    )
    loop = asyncio.get_event_loop()
    loop.run_until_complete(
        amain(proxies, check_url, result_proxy_file, concurrency, timeout)
    )
